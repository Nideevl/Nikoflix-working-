...
export default function MovieEditor({ movie }: { movie: any }) {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BASE}/admin/content/${movie.content_id}/genres`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const updateMovie = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE}/admin/movies/${form.content_id}`,
        {
          method: "PUT",
          headers: {
  ...
  const deleteMovie = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE}/admin/movie/${form.content_id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
